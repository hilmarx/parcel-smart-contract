...
contract('AllowanceModule delegate', function(accounts) {
    ...
    it('Execute multiple ether allowance with delegate', async () => {
        ...
        let setAllowanceData = await safeModule.contract.methods.setAllowance(lw.accounts[4], ETH_ADDRESS, web3.utils.toWei("1.0", 'ether'), 0, 0).encodeABI()
        await execTransaction(safeModule.address, 0, setAllowanceData, CALL, "set allowance")
        ...
        assert.equal(await web3.eth.getBalance(gnosisSafe.address), web3.utils.toWei("1.0", 'ether'))
        assert.equal(await web3.eth.getBalance(lw.accounts[4]), 0)

        let nonce = tokenAllowance[4]
        let transferHash = await safeModule.generateTransferHash(
            gnosisSafe.address, ETH_ADDRESS, lw.accounts[4], web3.utils.toWei("0.001", 'ether'), ADDRESS_0, 0, nonce
        )
        let signature = utils.signTransaction(lw, [lw.accounts[4]], transferHash)
        ...
        utils.logGasUsage(
            'executeAllowanceTransfer',
            await safeModule.executeAllowanceTransfer(
                gnosisSafe.address, ETH_ADDRESS, lw.accounts[4], web3.utils.toWei("0.001", 'ether'), ADDRESS_0, 0, lw.accounts[4], signature
            )
        )

        assert.equal(await web3.eth.getBalance(gnosisSafe.address), web3.utils.toWei("0.999", 'ether'))
        assert.equal(await web3.eth.getBalance(lw.accounts[4]), web3.utils.toWei("0.001", 'ether'))
        ...
        assert.equal(await web3.eth.getBalance(lw.accounts[4]), web3.utils.toWei("0.002", 'ether'))
